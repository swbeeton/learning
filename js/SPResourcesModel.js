;( function ( global ) {
    ...
    var LDSPResourcesModel = function ( options, cb ) {		
		return new LDSPResourcesModel.init( options, cb );                       
    }
	...
	var options = {
		...
		viewFields: ["Category_HeadlineA", "Category_HeadlineB"],
		orderBy: "",
		orderAsc: true,
		pagingInfo: "",
		pageIndex: 1,
    ...
    LDSPResourcesModel.init = function ( options, cb ) {
		
		this.setSPContext(options.siteUrl);
		this.validateQueryOptions(options).setQueryOptions(options);
		this.getResources( cb ); 
		return this 
		
	}
    
    // The following mimics a Constructor (Part 3)
    LDSPResourcesModel.init.prototype = LDSPResourcesModel.prototype;
    
    // Sets LDSPResourcesModel and U$ to be an alias of of this class in the global namespace 
    global.LDSPResourcesModel = global.R$ = LDSPResourcesModel;
